...
namespace ArgumentsParser
{
    public class Arguments
    {
        ...
        public Arguments(string description, Assembly programAssembly)
            : this()
        {
            UsagePrinter = new UsagePrinter();
        ...
        public void Parse(string[] rawArguments, ArgumentParseOptions parseOptions = ArgumentParseOptions.None)
        {
            ...
            while (tokens.Advance())
            {
                ...
                IArgument matchingArgument = args.FirstOrDefault(arg => arg.ShortName == argumentName || arg.LongName == argumentName);
                ...
                else if (tokens.Advance())
                {
                    ...
                    if (TryGetActualValue(tokens.CurrentToken, matchingArgument, out actualValue))
                    {
                        ...
                        Errors.Add(string.Format("invalid argument value for {0}: {1}", argumentNameToken, tokens.CurrentToken));
                }
                else
        ...
        private void PrintArgs()
        {
            foreach (IArgument arg in _args)
            {
                ...
                else
                {
                    ...
                    if (!(arg.Type.IsValueType && Equals(arg.DefaultValue, Activator.CreateInstance(arg.Type))) && arg.DefaultValue != null)
                    {
